...
const MARVEL_URL = `https://dog.ceo/api/breeds/list/all`
...
// const timeStamp = 1;
// const privateKey = 'a1e136aeb0482e6d800c2cf437431294bec6935a';
// const publicKey = 'c85aa1c0789c5600dec8243c2ddc0f62';
...
fetch(MARVEL_URL)
    ...
    .then(data => {
        const marvelObject = data.message; //Turn message into an object
        const breedsArray = Object.keys(marvelObject) //Turn object into an array

        for (let i = 0; i < breedsArray.length; i++) {
            const option = document.createElement('option'); //for option
            option.value = breedsArray[i]; //option value ='character
            option.innerText = breedsArray[i]; //<option value ='character'> character</option>
            select.appendChild(option); //adds current <option> tag to the select box list of optionss
...
const getDogoImg = url => {
    fetch(url)  //Going to the API irl above
        .then(res => {
        ...
        .then(data => {
            ...
            console.log(data.message); //extract message from JSON and attach to img tag as new source
        })
}
...
const doggoInfo = {
    ...
    assignName(array) {
        ...
        document.getElementById("character-name").innerHTML = `I am ${this.rname}`
    },
